...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1t58veLEnL?p=139)
...
### 3.1.2操作可行性
图书馆预约占座系统是根据用户经常使用的页面操作流程来进行设计的，并且页面保证统一，从视觉角度和操作角度上都能达到使用要求。
### 3.1.3经济可行性
在本次开发过程中，因为需要通过电脑来进行配置开发的环境，通过对技术的分析，发现目前正在使用的电脑是可以满足开发需要的，并不需要太多的金钱对电脑进行更换。所以，从经济角度上分析，可以满足开发要求。

从以上三个角度来进行分析论证，证明了图书馆预约占座系统是可以正常开发并且使用的。
## 3.2系统流程分析
...
管理员用例图的绘制结果见图3-5。管理员登录进入本系统操作的功能包括管理对学生的警告信息，管理房间信息，管理房间占座信息，管理论坛，管理留言版，学生和公告等。

![](/md/blog.005.png)

图3-5 管理员用例图

学生用例图的绘制结果见图3-6。学生登录进入本系统操作的功能包括查看房间，查询房间，选择座位，预约房间，发布帖子，评论帖子，查看警告信息，发布留言，查看回复等。
...
管理员功能结构图的绘制结果见图4-1。管理员登录进入本系统操作的功能包括管理对学生的警告信息，管理房间信息，管理房间占座信息，管理论坛，管理留言版，学生和公告等。

![](/md/blog.007.png)

图4-1 管理员功能结构图

学生功能结构图的绘制结果见图4-2。学生登录进入本系统操作的功能包括查看房间，查询房间，选择座位，预约房间，发布帖子，评论帖子，查看警告信息，发布留言，查看回复等。
...
（1）使用Visio这样的常用的实体属性图绘制工具来绘制留言实体属性图，绘制结果见图4-3。
...
通过上节内容可以知晓数据库中的各个实体，并通过一定方式把这些实体表示的内容进行数据表的转换，通常来说，每个实体都会对应一张具体的数据表，在本系统指定的数据库中创建命名好的数据库，才可以对数据表进行创建与设计。图书馆预约占座系统数据表设计结果展示如下：
...
|fangjian\_clicknum|点击次数|int(11)|是|
...
|buy\_zuowei\_number|占的座位|varchar(200)|是|
|buy\_zuowei\_time|占座日期|date|是|
|insert\_time|占座时间|timestamp|是|
|create\_time|创建时间 |timestamp|是|
表4-3 论坛表
...
|id (主键)|主键|int(11)|否|
|xuesheng\_id|学生|int(11)|是|
|jinggao\_name|警告标题 |varchar(200)|是|
|jinggao\_text|警告内容|text|是|
|insert\_time|警告时间|timestamp|是|
|create\_time|创建时间|timestamp|是|
表4-6 留言表
...
|xuesheng\_jinyong\_types|是否禁用|int(11)|是|
|create\_time|创建时间|timestamp|是|
...
## 5.1管理员功能实现
### 5.1.1 房间管理
管理员进入如图5-1所示的房间管理界面之后，管理员点击信息显示栏中最右侧的修改，删除，下架按钮可依次完成房间信息的修改，删除，下架操作。

![](/md/blog.015.png)

图5-1 房间管理界面
### 5.1.2 房间占座管理
管理员进入如图5-2所示的房间占座管理界面之后，管理员点击信息显示栏中最右侧的详情，删除按钮可依次完成房间占座信息的详情查看，删除等操作。管理员在当前界面也能查询房间占座信息。

![](/md/blog.016.png)

图5-2 房间占座管理界面
### 5.1.3 警告管理
管理员进入如图5-3所示的警告管理界面之后，管理员点击信息显示栏右侧的修改，删除按钮可依次完成警告信息的修改，删除等操作。警告信息包括警告标题，警告内容以及警告时间，警告学生等信息。

![](/md/blog.017.png)

图5-3 警告管理界面
### 5.1.4 留言版管理
管理员进入如图5-4所示的留言版管理界面之后，管理员查看学生留言内容，管理员回复学生的留言内容也能在本界面显示。
...
## 5.2学生功能实现
### 5.2.1 房间信息
学生进入如图5-5所示的房间信息界面之后，学生可以对需要的座位进行选择之后，然后提交预约信息。

![](/md/blog.019.png)

图5-5 房间信息界面
### 5.2.2 在线论坛
学生进入如图5-6所示的在线论坛界面之后，学生根据论坛帖子类型筛选论坛帖子，学生每次查看完帖子内容之后，就可以在当前界面的下方评论区域评论帖子。在在线论坛界面学生也能发布帖子。

![](/md/blog.020.png)

图5-6 在线论坛界面
### 5.2.3 留言版
学生进入如图5-7所示的留言版界面之后，学生在留言区域登记留言内容，在留言内容展示区域查看留言内容，查看回复内容等。
